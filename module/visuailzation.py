...
def trajectory_visualization(original_data:pd.DataFrame, analysis_data1:pd.DataFrame, cutoff:list, pixelmicron:float, resolution_multiplier=20, roi='', scalebar=True) -> np.ndarray:
    ...
    image = np.zeros((y_width, x_width, 4)).astype(np.uint8)
    image[:,:,3:4] = 255
