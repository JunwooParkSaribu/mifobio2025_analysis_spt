...
def preprocessing(data, pixelmicrons, framerate, cutoff=[3, 9999], div_time_gap=False, tamsd_calcul=True):
    ...
    analysis_data4 = {}
    analysis_data4[f'1d_displacements'] = []
    analysis_data4[f'state'] = []

    analysis_data5 = {}
    analysis_data5[f'ratios'] = []
        ...
        for i in range(len(time_chunk_idx) - 1):
            ...
            for i in range(len(state_chunk_idx) - 1):
                ...
                if cutoff[0] <= len(sub_trajectory) <= cutoff[1]:
                    ...
                    fr_K *= (pixelmicrons**2/framerate**fr_H) #TODO: check again
                    ...
                    x_disps = (sub_trajectory.x.iloc[1:].to_numpy() - sub_trajectory.x.iloc[:-1].to_numpy()) 
                    y_disps = (sub_trajectory.y.iloc[1:].to_numpy() - sub_trajectory.y.iloc[:-1].to_numpy())
                    ...
                    ratios = np.concatenate([(x_vec[1:] / x_vec[:-1]), (y_vec[1:] / y_vec[:-1])])
                    ...
                    analysis_data4[f'1d_displacements'].extend(list(np.concatenate((x_disps, y_disps))))
                    analysis_data4[f'state'].extend([sub_trajectory.state.iloc[0]] * len(list(np.concatenate((x_disps, y_disps)))))

                    analysis_data5[f'ratios'].extend(list(ratios))
    ...
    analysis_data4 = pd.DataFrame(analysis_data4)
...
def count_cumul_trajs_with_roi(data:pd.DataFrame|str, roi_file:str|None, start_frame=1, end_frame=100, cutoff=[5, 9999]):
    ...
    if type(data) is str and '.h5' in data:
        ...
        for traj_idx in traj_indices:
            ...
            if t in coords and cutoff[0] <= len(single_traj) <= cutoff[1]:
                coords[t].append([x, y])
        ...
        for trajectory in trajectory_list:
            ...
            if t in coords and cutoff[0] <= len(xyz) <= cutoff[1]:
                coords[t].append([x, y])
            observed_max_t = max(t, observed_max_t)
            observed_min_t = min(t, observed_min_t)
    else:
